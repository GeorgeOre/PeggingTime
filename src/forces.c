...
static void being_destructive_collision(body_t *body1, body_t *body2,
...
void physics_collision_handler(body_t *body1, body_t *body2, vector_t axis,
                               void *aux, double force_const) {
  // Impulse in elastic collision : J = (m1*m2)/(m1+m2)(1+C_r)(u_2 - u1)
  double mass1 = body_get_mass(body1);
  double mass2 = body_get_mass(body2);
  // get the components of the relative velocity on the collision axis
  vector_t relative_velo =
      vec_subtract(body_get_velocity(body2), body_get_velocity(body1));
  double relative_velo_axis = vec_dot(relative_velo, axis);
  // Check if one of the bodies is a wall (INFINITY), if so the reduced mass is
  // just the non-wall body's mass. Does not handle if both masses are INFINITY,
  // but this should never happen.
  double impulse_scalar = relative_velo_axis * (1 + force_const);
  vector_t impulse;
  double reduced_mass;
  if (mass1 == INFINITY) {
    reduced_mass = mass2;
    impulse = vec_multiply(reduced_mass * impulse_scalar, axis);
